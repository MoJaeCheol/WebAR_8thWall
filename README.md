...
`reality.intrinsics` 는 엔진의 `GraphicsCamera` 투영행렬로 **화면 종횡비에 맞춰 한 축이 잘린**
상태다(바이너리에서 확인). 반면 `CameraPixelArray` 는 **잘리지 않은 전체 프레임**을 준다.
잘린 화각의 초점거리를 전체 프레임에 적용하면 fx 가 크롭 배수만큼 과대평가되고,
Immersal 의 PnP 는 카메라를 그만큼 멀리 놓는다.

두 가지로 대응한다.

1. 카메라 픽셀은 정사각형이므로 `fx = fy` 여야 하고, 크롭은 한 축의 화각만 좁힌다.
   → 두 축에서 뽑은 값 중 **작은 쪽**이 잘리지 않은 축의 참값이다.
2. 그래도 남는 오차는 **측정된 스케일 비율이 곧 초점거리 오차 배수**이므로 그 값으로 나눈다.
   편차가 작을 때(계통 오차일 때)만 적용하고, 적용 후 표본을 비운다(옛 초점거리 값이 섞이면 안 됨).

합성 검증: 1.62배 오차를 넣으면 **한 번에 계수 0.617 (= 1/1.62) 로 수렴**하고 이후 스케일 1.000.
랜덤 오차만 있는 경우에는 계통으로 오판하지 않아 보정이 적용되지 않는다.
