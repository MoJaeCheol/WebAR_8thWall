...
VPS 의 요점은 **누가 언제 와도 같은 자리, 같은 각도**다.
그래서 배치 좌표는 기기가 아니라 **서버**에 있고, 관람 모드에서는 탭으로 옮길 수 없다.

| | 관람 모드 (기본) | 저작 모드 (`?edit=1`) |
|---|---|---|
| 바닥 탭 | 무시 | 문을 그 자리로 이동 |
| 회전 | — | `↺ 15°` / `15° ↻` 버튼 |
| 저장 | — | `💾 저장` → 서버에 기록 |
| 측위 성공 시 | 저장된 좌표에 자동 배치 | 저장된 좌표에서 시작 후 수정 |

**현장 저작 절차**

1. 스캔한 장소에서 `https://<주소>/?edit=1&debug=1` 접속
2. 측위 성공을 기다린다 (맵 좌표 기준이라 측위 전엔 저장 불가)
3. 바닥을 탭해 문을 원하는 자리로 옮긴다
4. `↺ / ↻` 로 각도를 맞춘다 — 화면의 빨간 칩에 현재 좌표·각도가 실시간으로 뜬다
5. `💾 저장`
6. 이후 **누구든 그냥 주소만 열면** 그 자리에 그 각도로 나타난다

**API**

```
GET  /api/anchor/:mapId    저장된 배치 좌표 조회
POST /api/anchor/:mapId    {position:{x,y,z}, rotationY} 저장
GET  /api/anchors          전체 덤프 (커밋용)
```

> ⚠ **Render 무료 플랜은 파일시스템이 휘발성이다.** 저장한 좌표는 재배포·재시작 시 사라진다.
> 확정된 좌표는 `GET /api/anchors` 로 덤프해서 `data/anchors.json` 에 커밋해야 영구 보존된다.
> 저장할 때 서버 로그에도 커밋용 JSON 이 통째로 찍힌다.

맵 ID 별로 저장되므로 여러 장소(교토·오사카 등)를 각각 저작해 둘 수 있다.
...
`poseConvention: 'auto'` (기본값) 면 규약을 손으로 고를 필요가 없다.

**원리** — Immersal 맵도 8th Wall SLAM 도 중력 정렬(Y 위)이다.
따라서 올바른 맵→트래킹 변환은 **yaw(Y축 회전)만** 있어야 하고 기울기는 0 이어야 한다.
규약이 틀리면 큰 기울기가 생기므로, 후보 중 기울기가 가장 작은 것이 정답이다.

측위 성공 시 로그에 후보별 기울기가 찍힌다:

```
[immersal]   후보 0:그대로 기울기 0.0°
[immersal]   후보 1:전치 기울기 30.8°
[immersal]   후보 2:YZ반전 기울기 164.9°
[immersal]   후보 3:YZ반전+전치 기울기 156.8°
[immersal] 규약 자동 선택 → 0:그대로 (기울기 0.0°)
```

정답과 오답의 차이가 0° 대 23°~165° 로 벌어져서 애매한 경우가 없다.
합성 데이터로 4개 규약 모두 100% 판별을 확인했다.

`gravityLock: true` (기본값) 면 남은 pitch/roll 을 버리고 yaw 만 쓴다.
두 좌표계가 모두 중력 정렬이므로 기울기는 측위 오차일 뿐이고, 버리는 편이 안정적이다.

수동으로 고정하려면 `poseConvention` 에 숫자(0~7)를 넣거나 HUD 의 **정렬 보정** 버튼을 쓴다
(버튼을 누른 순간부터 자동 선택은 꺼진다).
...
> ⚠ **프로덕션 캐시**: 앱 파일(css/js/html)은 `no-cache` 로 항상 재검증한다.
> 예전처럼 `max-age=1h` 를 걸면 고친 코드가 폰에 최대 1시간 안 내려가서 디버깅이 꼬인다.
> 장기 캐시는 파일명에 버전이 박힌 `external/` 벤더 스크립트에만 적용한다.
